...
namespace QuanLyCongDanThanhPho
{
    internal class MailDAO
    {
        ...
        {
            string sqlStr = string.Format($"SELECT MaMail as [Mã mail], TieuDe as [Tiêu đề], Ngay as [Ngày], NguoiGui as [Người gửi], NguoiNhan as [Người nhận], NoiDung as [Nội dung] FROM Mail WHERE NguoiGui = {nguoigui}");
            return DBConnection.Instance.LayDanhSach(sqlStr);
        }
        public DataTable LayDanhSachNhan(int nguoinhan)
        {
            string sqlStr = string.Format($"SELECT MaMail as [Mã mail], TieuDe as [Tiêu đề], Ngay as [Ngày], NguoiGui as [Người gửi], NguoiNhan as [Người nhận], NoiDung as [Nội dung] FROM Mail WHERE NguoiNhan = {nguoinhan}");
            return DBConnection.Instance.LayDanhSach(sqlStr);
        }
